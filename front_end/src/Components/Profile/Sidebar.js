  ...
  return (
    <div className='bg-zinc-800 lg:h-[100%] h-auto flex flex-col items-center justify-between p-4 rounded'>
      <div className='flex items-center flex-col justify-center'>
        {" "}
      <img className='h-[12vh] ' src={data.user_avtar} alt='img'></img>
      ...
      {
        role === "user" && (<div className='w-full flex-col items-center hidden justify-center  lg:flex'>
      ...
      {
        role === "admin" && (<div className='w-full flex-col items-center hidden justify-center my-14 py-10 lg:flex'>
          <Link to="/profile" className="text-zinc-100 font-semibold w-full py-2 duration-300 text-center hover:bg-zinc-900 rounded transition-all">All Order</Link>
      ...
      <button className='bg-zinc-900 w-3/6 lg:w-full mt-4 lg:mt-0 mt-5 text-white font-semibold flex items-center justify-center py-2 rounded trasition-all duration-300 ' onClick={() => {
      ...
      history("/")
      }}>Log out<FaArrowRightFromBracket  className='ms-2'/></button>
    </div>
  )
}
